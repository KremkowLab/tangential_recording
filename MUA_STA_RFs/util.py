...
def basic_peak_detector(sig, orientation = 'negative', thresh = -3.5, verbose = False):
    ...
    ind = (peak_ind if orientation == 'both' else peak_ind_pos
            if orientation == 'positive' else peak_ind_neg)
    if verbose:
        size = len(sig)
        n = len(ind)
...
def filter_detect(channel, start, thresh, orientation, low_cut, high_cut, sampling_rate, order = 2, 
                  verbose = False):
...
def align_timestamps(timestamps, timestamps_sync, reference_sync, verbose = False):
    ...
    if len(timestamps_sync) != len(reference_sync):
        if verbose:
            print("The timestamps_sync ({}) and reference_sync ({}) do not have same length.".format(len(timestamps_sync), len(reference_sync)))
...
def align_and_save_timestamps(to_be_aligned_sync_ch, ref_sync_ch, 
                              to_be_aligned_ttl_dir, ref_ttl_dir):
    """
    PARAMETERS
    ----------
    ...
    """
    timestamps_sync, timestamps = get_timestamps(to_be_aligned_ttl_dir, to_be_aligned_sync_ch)
