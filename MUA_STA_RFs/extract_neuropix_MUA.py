...
class extract_NP_MUA():
    ...
    _signal_orientation = 'negative'   # 'positive' or 'negative', the signal orientation for processing
    
    def __init__(self, stim_ttl_dir, raw_data_dir, save_dir, fname_extensions=(None, None), total_ch=384, spike_event_std_thresh=-4, 
                 extract_start_time=None, extract_stop_time=None, event_keys=[(1, 'sync'), (2, 'starts'), (3, 'frametimes'), (4, 'stops')], 
                 slice_len=None, align_to_probe_timestamps=True, stim_sync_ch=1, probe_sync_ch=1, probe_ttl_dir="", n_cores=mp.cpu_count()):
        ...
        if self.align_to_probe_timestamps:
            ...
            self.stim_ttl_dir = align_and_save_timestamps(self.stim_sync_ch, self.probe_sync_ch, self.stim_ttl_dir, self.probe_ttl_dir)
    ...
    def _get_filename(self, fname_extensions=None):
        ...
        self.stim_ttl_fname = "stim_TTLs.npy" if not prefix else prefix + "stim_TTLs.npy"
    ...
    def _get_stim_ttl(self):
        ...
        for ch, key in self.event_keys:
            ...
            if self.extract_stop_time:
                self.stim_ttl_dict[key] = ch_timestamps[np.where((ch_timestamps >= start_t) & (ch_timestamps <= self.extract_stop_time))]
            else:
                self.stim_ttl_dict[key] = ch_timestamps[np.where(ch_timestamps >= start_t)]
    
    def _start_extract_spikes(self):
        """Start extract the spike times slice-by-slice."""
        self._get_raw_data()
        self._flatten_timestamps()
        extract_start_idx = 0 if not self.extract_start_time else self.extract_start_time - self.data_start_time
        extract_stop_idx = self.raw_data.shape[1] if not self.extract_stop_time else self.extract_stop_time - self.data_start_time
        if self.slice_len:
            idx = np.arange(extract_start_idx, extract_stop_idx, self.slice_len * self.sampling_rate)
            idx = np.append(idx, extract_stop_idx)
        else:
            idx = np.array([extract_start_idx, extract_stop_idx])
        print("Slicing points (unit time): {}\nTotal number of slices: {}".format(idx, len(idx)-1))
        for n, i in enumerate(idx[:-1], 1):
            if not os.path.exists(os.path.join(self.save_dir, "slice{}.npy".format(n))):
                t1 = time.time()
                self._cur_start = i; self._cur_stop = idx[n]
                print("Current slice (unit time): {} to {}".format(self._cur_start, self._cur_stop))
                self._get_cur_data()
    ...
    def _get_raw_data(self):
        ...
        self.raw_data_timestamps = np.load(os.path.join(self.raw_data_dir, "timestamps.npy"))
    ...
    def _flatten_timestamps(self):
        ...
        self.data_start_time, self.total_timestamp_len = get_rawdata_timestamps_info(self.raw_data_timestamps)
        start_zero = self.data_start_time == 0
        correct_ideal_len = self.raw_data.shape[1] - self.total_timestamp_len == 0
        correct_time_len = self.total_timestamp_len - self.raw_data_timestamps.shape[0] == 0
        if start_zero:
            self.raw_data_timestamps = np.arange(self.raw_data.shape[1])
        else:
            if not correct_ideal_len:
                if self.data_start_time + self.total_timestamp_len == self.raw_data.shape[1]:
                    self.raw_data_timestamps = np.arange(self.raw_data.shape[1])
                else:
                    get_action("First timestamp (data_start_time): {}\n"
                               "The raw data ({}) and timestamps ({}) lengths"
                               " are not consistent!".format(self.data_start_time, self.raw_data.shape[1], 
                                                             self.total_timestamp_len))
            else:
                if not correct_time_len:
                    self.raw_data_timestamps = np.arange(self.data_start_time, self.data_start_time+self.raw_data.shape[1])
    ...
    def _get_cur_data(self):
        ...
        if os.path.exists(tmp_file):
            try:
                ...
                pass
        self.cur_data = np.memmap(os.path.join(self.save_dir, "temp.npy"), dtype = np.int16, mode = "w+", 
    ...
    def _get_spiketimes(self, slice_idx):
        ...
        spiketimes_arr = (Parallel(n_jobs=self._n_cores, verbose=10)
                          (delayed(filter_detect)(self.cur_data[ch - 1,:], 
                                           self.raw_data_timestamps[self._cur_start], 
                                           self.spike_event_std_thresh, self._signal_orientation, self._signal_lowcut, 
                                           self._signal_highcut, self._sampling_rate, self._butter_bandpass_order)
    ...
    def _save_spiketimes_dict(self, num_files):
        ...
        for i in range(num_files):
            ...
            for ch in range(1, self.total_ch + 1):
                self.st_dict[ch] = np.append(self.st_dict[ch], st_arr[ch - 1]).astype(int)
            os.remove(cur_file)
    ...
    def _median_subtraction(self, axis):
        ...
        Parallel(n_jobs = self._n_cores)(delayed(self._parallel_median)
                                        (idx[i], idx[i+1], axis) for i in range(self._n_cores))
